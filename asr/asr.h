...
public:
    ...
    /* 获取识别结果 */
    void getTheResult(QString fileName);    //--供外部调用访问，用于触发申请数据（不像其余以槽函数形式，这里没设置信号，所以作为直接调用）
...
private slots:
    ...
    /* 准备读取响应返回来的数据 */
    void readyReadData();   //--内部槽函数，用于数据返回后处理

    /* 响应完成处理 */
    void replyFinished();   //--内部槽函数，处理完成

signals:
    void asrReadyData(QString); //--像外部发送读取信号
